...
func (e *BaseExecutor) GenerateWitness(verifiedBlockNumber int64) (witness *prove.TxWitness, err error) {
	keys := e.witnessKeys
	...
	for _, ac := range witness.AccountsInfoBefore {
		...
		if ac.AccountIndex >= int64(len(e.bc.StateDB().AccountAssetTrees)) {
			assetTree, err = tree.NewEmptyAccountAssetTree(e.bc.StateDB().TreeCtx, ac.AccountIndex, uint64(verifiedBlockNumber))
			if err != nil {
				return nil, err
			}
		} else {
